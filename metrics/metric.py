...
from metrics.precision_and_recall import knn_precision_recall_features
...
parser.add_argument(
    ...
    type=str,
    nargs=2,
    help=("real datasets, generated datasets by order"),
)
parser.add_argument(
    "--metric_type",
    type=str,
...
def get_activations(
    ...
    for batch in tqdm(dataloader):
        ...
        if dims == 768:
            pred = pred[:, 0:7, :, :].reshape(len(pred), -1).cpu().numpy()
        else:
...
def compute_statistics_of_path(
    path, model, batch_size, dims, device, num_workers=1, metric_type="fid"
):
    ...
    return calculate_activation_statistics(
        files, model, batch_size, dims, device, num_workers, metric_type
    )


def compute_feature_of_path(path, model, batch_size, dims, device, num_workers=1):
    path = pathlib.Path(path)
    files = sorted(
        [file for ext in IMAGE_EXTENSIONS for file in path.glob("*.{}".format(ext))]
    )
    features = calculate_activation_statistics(
...
def calculate_fid_given_paths(
    paths, batch_size, device, dims, num_workers=1, metric_type="fid"
):
    """Calculates the FID of two paths"""

    if metric_type not in ["fid", "sfid"]:
        raise ValueError(f"invalid name {metric_type}, use fid or sfid.")
    if metric_type == "sfid":
        assert dims == 768
    if metric_type == "fid":
        assert dims == 2048
    ...
    m1, s1 = compute_statistics_of_path(
        paths[0], model, batch_size, dims, device, num_workers, metric_type
    )
    m2, s2 = compute_statistics_of_path(
        paths[1], model, batch_size, dims, device, num_workers, metric_type
    )
...
def calculate_PR_given_paths(
    paths, batch_size, device, dims, num_workers=1, metric_type="PR"
):
    ...
    ref_features = compute_statistics_of_path(
        paths[0], model, batch_size, dims, device, num_workers, "PR"
    )
    eval_features = compute_statistics_of_path(
        paths[1], model, batch_size, dims, device, num_workers, "PR"
    )
    
    del model
    status = knn_precision_recall_features(ref_features, eval_features)

    return status


def calculate_metrics(paths, batch_size, device, num_workers=1):
    real_path = paths[0]
...
def main():
    ...
    lpips_score, ssim_score, psnr_score = calculate_metrics(args.paths, args.batch_size, device, num_workers)
    
    with open(os.path.join(os.path.dirname(args.paths[1]), f"{os.path.basename(args.paths[1])}_metrics.txt"), "w") as f:
        f.writelines(f'lpips: {lpips_score}\r\n')
        f.writelines(f'ssim: {ssim_score}\r\n')
        f.writelines(f'psnr: {psnr_score}\r\n')
        f.write(f"args: {args}\r\n")
